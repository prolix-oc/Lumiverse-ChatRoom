...
function setup(ctx) {
  ...
  configCard.appendChild(createSettingRow("Generation Connection Profile", "The LLM connection used to generate council messages.", connectionMount));
  ...
  configCard.appendChild(createSettingRow("Chatroom Persona", "Persona used to represent you in the chatroom. Defaults to your currently active persona; pick another to override it just for this extension.", personaMount));
  ...
  timeSettingsGroup.appendChild(createSettingRow("Time Between Messages (seconds)", "How long to wait before generating the next council message.", messageIntervalMount));
  ...
  timeSettingsGroup.appendChild(createSettingRow("Random Interval", "When enabled, the delay between messages varies randomly within the range below.", randomToggleMount));
  ...
  messagesSettingsGroup.appendChild(createSettingRow("Messages Between Responses", "How many story chat messages must be sent before the council auto-replies.", messageCountMount));
  ...
  messagesSettingsGroup.appendChild(createSettingRow("Random Message Count", "When enabled, the number of messages required varies randomly within the range below.", randomMessageCountMount));
  ...
  configCard.appendChild(createSettingRow("Context Retrieval (messages)", "How many recent story messages the council can see before reacting.", contextMount));
  ...
  configCard.appendChild(createSettingRow("Max Context Tokens", "Maximum tokens the council chatroom history can consume. Older messages are removed automatically when this limit is exceeded.", maxContextTokensMount));
  ...
  advancedGenerationGroup.appendChild(createSettingRow("Temperature", "Higher values increase randomness. Leave blank to use the default of 1. A value of 0 omits temperature from the request.", temperatureMount));
  ...
  advancedGenerationGroup.appendChild(createSettingRow("Top P", "Nucleus sampling cutoff. Leave blank to use the default of 0.95. A value of 0 omits top_p from the request.", topPMount));
  ...
  advancedGenerationGroup.appendChild(createSettingRow("Max Response Tokens", "Maximum completion tokens to request. Leave blank to use the default of 8192.", maxResponseTokensMount));
  ...
  function setCouncilMembers(nextMembers) {
  ...
  function highlightRenderedMentions(html) {
    const candidateNames = new Set;
    if (userPersona?.name?.trim())
      candidateNames.add(userPersona.name.trim());
    for (const member of councilMembers) {
      if (member.name.trim())
        candidateNames.add(member.name.trim());
    }
    ...
    return html.replace(pattern, (_match, prefix, name) => `${prefix}<span class="chatroom-inline-mention">@${name}</span>`);
  }
  ...
  function applyKeyboardAdjustment(insetBottom, viewportHeight) {
    const pos = widget.getPosition();
  ...
  let nativeCollapsedDragStart = null;
  const isNativeCompactCollapsed = () => isDesktopWidgetPopout && usesCompactWidgetShape() && isCollapsed && !isFullscreen;
  ...
  headerIcon.addEventListener("click", (event) => {
    ...
    nativeCollapsedDragStart = null;
    if (dragStart && Math.hypot(event.screenX - dragStart.x, event.screenY - dragStart.y) >= NATIVE_COLLAPSED_DRAG_THRESHOLD) {
      event.preventDefault();
      event.stopPropagation();
      return;
    }
  ...
  `;
  const stopDragInit = (e) => e.stopPropagation();
  ...
  function refreshMentionResults() {
    ...
    mentionResults = councilMembers.filter((member) => !normalizedQuery || member.slug.includes(normalizedQuery) || member.name.toLowerCase().includes(normalizedQuery)).sort((left, right) => {
  ...
  function applyWidgetTheme() {
    ...
    inputShell.style.background = glassEnabled ? "color-mix(in srgb, var(--lumiverse-fill-subtle) 82%, var(--lcs-glass-bg, transparent) 18%)" : "var(--lumiverse-fill-subtle)";
    inputShell.style.boxShadow = glassEnabled ? "inset 0 0 0 1px var(--lcs-glass-border, var(--lumiverse-border))" : "inset 0 0 0 1px var(--lumiverse-border)";
  ...
  function normalizeOutgoingMentions(rawText) {
    const slugToMember = new Map(councilMembers.map((member) => [member.slug, member]));
    const mentionedMemberNames = [];
    const seen = new Set;
    const content = rawText.replace(/(^|\s)@([a-z0-9][a-z0-9-]*)(?=\s|$|[.,!?;:])/gi, (_match, lead, rawSlug) => {
  ...
  const sendMessage = () => {
    ...
    renderMirrorContent();
    closeMentionPopover();
    appendMessage(userPersona?.name || "You", userPersona?.name || "You", content, userPersona?.avatarUrl || null, true, clientMessageId);
  ...
  const unsubBackend = ctx.onBackendMessage((payload) => {
    if (payload.type === "settings_loaded") {
      ...
      connectionSelect.update({
        options: Array.isArray(payload.connections) ? payload.connections.map((c) => ({
          value: c.id,
      ...
      personaSelect.update({
        options: Array.isArray(payload.personas) ? payload.personas.map((p) => {
          const initial = (p.name?.[0] || "?").toUpperCase();
          return {
            value: p.id,
            label: p.name,
            sublabel: p.title || undefined,
            leading: p.avatarUrl ? { type: "image", src: p.avatarUrl, fallback: { text: initial } } : { type: "initial", text: initial }
          };
        }) : [],
